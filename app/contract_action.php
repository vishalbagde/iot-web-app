...
if(isset($_POST['btn_action']))
{
	...
	if($_POST['btn_action'] == 'fetch_single')
	{
		$obj = new ContractMstr();
		$obj->openConnection();
		$obj->setWhere("contract_id = ".$_POST["contract_id"]." ");
		$query =  $obj->preparedExecuteNoneQuery(Common::$select);
		$run = $obj->executeReader($query);
		while($row=mysqli_fetch_array($run))
		{
			$output=$row;
			$output['amc_details']=fill_amc_list($row['contract_type']);
		}
	...
	if($_POST['btn_action'] == 'Edit')
	{
		$obj = new ServiceMstr();
		$obj->openConnection();
		$query = "select * from service_mstr where contract_id = ".$_POST['contract_id']." and service_status <> 'pending'";
		$run = $obj->executeReader($query);		
		
		if($obj->getTotalRows() > 0)
		{
			echo '<span class="text text-danger">Service are full fill not possible to update AMC</span>';
		}
		else
		{
			$obj = new ContractMstr();
